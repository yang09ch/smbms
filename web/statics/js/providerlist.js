...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/provide/delete",
		data:{method:"delprovider",proid:obj.attr("proid")},
		dataType:"text",
		success:function(result){
			if(result== "true"){//删除成功：移除删除行
				cancleBtn();
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/provide/toSelect/"+ obj.attr("proid")+"/1";
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provide/toSelect/"+ obj.attr("proid")+"/2";
	});
