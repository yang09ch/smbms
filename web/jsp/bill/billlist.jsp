...
<div class="right">
       ...
       <div class="search">
       <form method="post" action="${pageContext.request.contextPath }/bill/limit">
			<input name="method" value="query" class="input-text" type="hidden">
			<span>商品名称：</span>
			<input name="queryProductName" type="text" value="${param.queryProductName }">
			...
			<select name="queryProviderId">
				<c:if test="${providerList != null }">
				   ...
				   <c:forEach var="provider" items="${providerList}">
				   		<option
				   		value="${provider.id}" ${provider.id == param.queryProviderId?"selected":"" }>
			...
			<select name="queryIsPayment">
				<option value="-1">--请选择--</option>
				<option value="1" ${param.queryIsPayment == 1 ? "selected=\"selected\"":"" }>未付款</option>
				<option value="2" ${param.queryIsPayment == 2 ? "selected=\"selected\"":"" }>已付款</option>
       		</select>
    ...
    <c:import url="../rollpage.jsp">
        <c:param name="totalCount" value="${billList.totalCount}"/>
        <c:param name="currentPageNo" value="${billList.pageIndex}"/>
